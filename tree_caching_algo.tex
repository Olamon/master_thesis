...
separation by defining $X'_i = X_i \setminus \bigcup_{j=1}^{i-1}$. Note that 
$X'_i$ is tree cap of $X_i$. The sequence $X'_1, X'_2, \ldots, X'_m$ is 
pairwise disjoint and covers $C$ - $C \subseteq \bigsqcup_{j=1}^m X'_j$. Let 
$X_j^D = D \cap X'_j$, then $D = \bigsqcup_{j=1}^m X_j^D$ since $D \subsetq C$ 
and $X_j^D$ is tree cup of $X'_j$ so, by transitivity of relation of being 
treecap, $X_j^D$ makes up a tree cap of $X_j$. From Lemma 
\end{proof}
...
\myparagraph{Upper bound for open fileds}
