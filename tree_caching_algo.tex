...
The \textbf{TRC} proceeds in phases. The first phase starts at time 0 and at the
...
values for $v \in T$.  \If{exists a valid changeset $X$ such that: \begin{itemize}
...
To achieve such a $P'$, we will change $P$ by the application of \textit{legal
...
The lemma above provides us with a helpful attribute of the fields. Instinctively, for
...
Now we can present a lemma which will be then used as the induction step in the
...
Now, the above lemma, when used as an induction step, proves Theorem
...
\begin{lemma} Fix positive field $F^t$ with corresponding changeset $X_t$ and node $v \in X_t$
...
\end{proof} 
\subsubsection{Competitiveness of TRC} We will use the results from the
...
\subsection{Lower bound for the competitive ratio}
