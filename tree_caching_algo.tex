...
of all fields (excluding the open one) which we denoted by $|\mathcl{F}|$. It 
implies that $|\mathcl{F}| = cnt_{OUT} + cnt_{IN}$ so $cnt_{OUT} = 
\frac{|\mathcl{F}| + k_P}{2} \geq \frac{|\mathcl{F}| + k_P}{2}.$
...
$\frac{1}{2h(T)}$ throughout $OUT$ periods paired with $IN$ periods such that 
have at least $\frac{\alpha}{2}$ requests. Therefore we have at least 
$(cnt_{OUT} - k_P)
\end{proof}
